...
static struct {
    ...
    size_t size;
    int handle_counter;
    bool should_fail_set;
    bool should_fail_commit;
} s_nvs_mock = { .size = 0, .handle_counter = 1 };

/* Control injection from tests */
void mock_nvs_set_fail_set(bool fail) { s_nvs_mock.should_fail_set = fail; }
void mock_nvs_set_fail_commit(bool fail) { s_nvs_mock.should_fail_commit = fail; }
...
int nvs_set_blob(nvs_handle_t handle, const char *key, const void *value, size_t length) {
    (void)handle;
    (void)key;
    if (s_nvs_mock.should_fail_set) return ESP_ERR_NVS_NO_FREE_PAGES;
...
int nvs_commit(nvs_handle_t handle) {
    (void)handle;
    if (s_nvs_mock.should_fail_commit) return ESP_ERR_NVS_NO_FREE_PAGES;
    return 0;
}
MOCK_EOF
...
void mock_nvs_set_fail_set(bool fail);
void mock_nvs_set_fail_commit(bool fail);
#endif
MOCK_EOF
...
/* device_app_schedule_restart provided by test_fault_injection.c */
MOCK_EOF
...
cc -std=c11 -Wall -Wextra -Werror -O2 \
   ...
   "$DIR/test_fault_injection.c" \
   -o "$OUT/test_fault_injection"

"$OUT/test_fault_injection"
