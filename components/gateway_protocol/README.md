...
ESP-GATT Protocol v2 wire contract for the Device side. Shared dependency:
...
- Protocol/UUID constants: service `0xABF0`, COMMAND `0xABF1`,
  STATUS `0xABF2`, CCCD `0x2902`, `GW_PROTOCOL_VERSION = 2`.
- `gw_message_t`: wire message model (giống hệt `gw_message_t` Gateway).
- CBOR encode/decode với numeric keys (0..10).
...
- `gw_message_init` — zero-init, TX mặc định emit v2 explicit.
...
- `gw_message_encode(msg, buf, cap)` → độ dài hoặc `-gw_result_t`.
  ...
  optional: `device_id, request_id, name, device_type, ble_addr(+type)`.
- `gw_message_decode(buf, len, msg)` — strict như decoder Gateway:
  bắt buộc `type/command/int_value/bool_value`; version 1..2 (thiếu → v2);
